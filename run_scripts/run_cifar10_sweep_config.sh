...
LOCAL_EPOCHS="$4"
SEED="$5"
EXP_NAME=GP_"${DATASET}"_epsilon_"${EPS}"_clip_"${CLIPPING_BOUND}"_seed_"${SEED}"
# Print the argument
...
CUDA_VISIBLE_DEVICES=$GPU python main_base_gp.py "${ARGUMENTS[@]}" >> "${LOG_FOLDER}"/FedAvgSgdDP_GP_"${EXP_NAME}".txt
...
CUDA_VISIBLE_DEVICES=$GPU python main_base_gep_gp.py "${ARGUMENTS[@]}" "${GEP_ARGUMENTS[@]}"  >> "${LOG_FOLDER}"/FedAvgGep_GP_history_"${HISTORY_SIZE}"_"${EXP_NAME}".txt
