...
from net import mnistNet, cifar10Net, SVHNNet, EMGModel, save_checkpoint, load_checkpoint
...
def main():
    ...
    # raise Exception(f'num_param_list {num_param_list} num_bases_list {num_bases_list} sqrt_num_param_list {sqrt_num_param_list} num_basis_elements_actual {num_basis_elements_actual}')
    # <<< *** GEP

    pbar = trange(base_epoch, global_epoch)
    for epoch in pbar:
        ...
        public_clients_model_updates_not_used = {n: [] for n, _ in global_model.named_parameters()}
        ...
        # public_grads_list_not_used = [torch.stack(public_clients_model_updates_not_used[n]) for n, p in
        ...
        #     public_grads_flat = public_grads_flat[mask]
        # pub_grads_nan_rows = float(public_grads_flat.size(0) - public_grads_flat.size(0))
        # flatten grads for clipping and noising
        # public_grads_flat_not_used = flatten_tensor(public_grads_list_not_used)
        # assert public_grads_flat_not_used.dim() == 2, f'Expected flat grads per batch'
        # assert public_grads_flat_not_used.shape[0] == len(public_clients_loader), f'Expected grad per batch'
        # assert public_grads_flat_not_used.shape[
        #            1] == num_trainable_params, f'Expected {num_trainable_params} element per grad'
        # pub_grad_not_used_mean_norm = norm_of_rows(public_grads_flat_not_used)
        # pub_grad_not_used_nonfiltered_size = public_grads_flat_not_used.size(0)
        # if torch.any(torch.isnan(public_grads_flat_not_used)):
        #     mask = ~torch.isnan(public_grads_flat_not_used).any(dim=1)
        #     public_grads_flat_not_used = public_grads_flat_not_used[mask]
        # pub_grad_not_used_filtered_size = public_grads_flat_not_used.size(0)
        # pub_grads_backpack_nan_rows = pub_grad_not_used_nonfiltered_size - pub_grad_not_used_filtered_size

        # X = public_grads_flat
        # mean = torch.mean(X, dim=0, keepdim=True)
        # scale = torch.std(X, dim=0, keepdim=True)
        # mx, _ = torch.max(X, dim=0, keepdim=True)
        # mn, _ = torch.min(X, dim=0, keepdim=True)
        #
        # raise Exception(f'public_grads_flat shape {public_grads_flat.shape}  mean: {float(mean.mean())} scale: {float(scale.mean())} '
        #                 f' max: {float(mx.mean())} min: {float(mn.mean())}')
        #
        ...
        # outliars_ammount = len(outlier_users_list[outlier_users_list == True])
        # if outliars_ammount > 0:
        #     # keep only the inliars
        #     inliar_mask = ~outlier_mask
        #     inliar_users_list = inliar_mask.any(dim=1)
        #     sampled_client_indices = [sampled_client_indices[i] for i in range(len(sampled_client_indices)) if
        #                               inliar_users_list[i]]
        #     grads_flattened = grads_flattened[inliar_users_list]
        #     sampled_client_num = grads_flattened.shape[0]
